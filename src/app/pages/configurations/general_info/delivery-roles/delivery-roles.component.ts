import { DeliveryRoles } from './../../../../models/delivery-roles';
import { DeliveryRolesFormComponent } from './delivery-roles-form/delivery-roles-form.component';
import { ChangeDetectionStrategy, ChangeDetectorRef, Component, OnInit } from '@angular/core';
import { TranslateService } from '@ngx-translate/core';
import { MessageService } from 'primeng/api';
import { DialogService } from 'primeng/dynamicdialog';
import { Observable, of, Subscription } from 'rxjs';
import { map } from 'rxjs/operators';
import { ApiService } from 'src/app/shared/services/api.service';

@Component({
  templateUrl: './delivery-roles.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DeliveryRolesComponent implements OnInit {

  closeDialogSubs: Subscription = new Subscription();
  tableCols: any = [];
  filters: any[] = [];
  tableTitle : string = this._translate.instant('delivery_roles');
  deliveryRolesList$: Observable<DeliveryRoles[] | any> = of([]);
  
  constructor(private _apiService :ApiService , private _translate: TranslateService, private readonly _dialogService: DialogService, private _cdr: ChangeDetectorRef, private readonly _messageService: MessageService) {
    this._apiService.apiName = 'delivery-rules'
   }

  ngOnInit(): void {
    this.initTableCols();
    this.loadAllDeliveryRoles();
  }

  private initTableCols = (): void => {
    this.tableCols = [
      { field: 'id', header: this._translate.instant('id') },
      { field: 'name', header: this._translate.instant('name') },
      { field: 'branches', header: this._translate.instant('branches') },
      { field: 'condition', header: this._translate.instant('condition') },
      { field: 'charge', header: this._translate.instant('charge') },
      { field: 'answer', header: this._translate.instant('answer') },
    ];
    this.filters = this.tableCols.map((el:any)=> el.field)
  }

  private loadAllDeliveryRoles = (): void => {
    this.deliveryRolesList$ = this._apiService.getAll().pipe(map(resp => resp.error_code === 0 && resp.data));
  }

  deleteDeliveryRoles= (data: DeliveryRoles): void => {
    this._apiService.delete(data.id).subscribe(res => {
      if(res.error_code === 0) {
        this.loadAllDeliveryRoles();
        this._cdr.detectChanges();
      }
    });
  }

  addNew = ():void =>{
    this.closeDialogSubs = this._dialogService.open(DeliveryRolesFormComponent, {
      header: this._translate.instant('add_delivery_roles'),
      width: '90%',
      height: '75%',
      contentStyle: { "overflow-y": "scroll" },
      baseZIndex: 10000,
    }).onClose.subscribe((added) => {
      if(added) {
        this.loadAllDeliveryRoles();
        this._messageService.add({ severity: 'success', summary: this._translate.instant('successfully_added') });
        this._cdr.detectChanges();
      }
    });
  }

  showEditDialog = async (e: any) => {
    const response = await this._apiService.getById(e.id).toPromise();
    if (response?.error_code === 0) {
      this._dialogService.open(DeliveryRolesFormComponent, {
        header: this._translate.instant('edit_delivery_rules'),
        width: '50%',
        contentStyle: { "overflow": "hidden" },
        baseZIndex: 10000,
        data: { editMode: true, details: response?.data },
        closable: true
      }).onClose.subscribe(edited => {
        if (edited) {
          this.loadAllDeliveryRoles();
          this._messageService.add({ severity: 'success', summary: this._translate.instant('successfully_updated') });
          this._cdr.detectChanges();
        }
      });
    }
  }

}

